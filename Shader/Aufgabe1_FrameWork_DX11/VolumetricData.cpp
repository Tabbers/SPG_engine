...
VolumetricData::VolumetricData(int Height, int Width, int Depth, DirectX::XMFLOAT3 CubeSize, DirectX::XMFLOAT3 CubeStep) :
...
*/
HRESULT VolumetricData::CreateTestData(const char type, ID3D11Device* device)
{
	...
	************/
	...
	HRESULT hr;
	if (FAILED(hr = createTexture(device)))
		return hr;

	if (FAILED(hr = createShaderResourceView(device)))
		return hr;
...
*/
HRESULT VolumetricData::CreateTriTableResource(ID3D11Device* device)
{
	...
	hr = device->CreateShaderResourceView(texture, NULL, &m_triTableSRV);
	return hr;
}
...
*/
HRESULT VolumetricData::createTexture(ID3D11Device* device)
{
	return device->CreateTexture3D(&m_texDesc, &m_subData, &m_texture);
}

/*
Creates the shader resource that the GPU can use directly
*/
HRESULT VolumetricData::createShaderResourceView(ID3D11Device* device)
{
	return device->CreateShaderResourceView(m_texture, NULL, &m_shaderResourceView);
}
...
*/
float VolumetricData::getDistance(DirectX::XMFLOAT3 p1, DirectX::XMFLOAT3 p2)
{
	DirectX::XMVECTOR vec1 = DirectX::XMLoadFloat3(&p1);
	DirectX::XMVECTOR vec2 = DirectX::XMLoadFloat3(&p2);
	DirectX::XMVECTOR length = DirectX::XMVector3Length(DirectX::XMVectorSubtract(vec1,vec2));
	...
	for (float z = -1; z < 1.0f; z += m_cubeStep.z)
	{
		for (float y = -1; y < 1.0f; y += m_cubeStep.y)
		{
			for (float x = -1; x < 1.0f; x += m_cubeStep.x)
			{
				...
				(*outVertices)[idx] =
					{ DirectX::XMFLOAT4(x, y, z, 1), DirectX::XMFLOAT4((y + 1.0f) / 2.0f, 0.0f, 0.0f, 1.0f) };
