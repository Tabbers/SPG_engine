...
DS_OUTPUT main(	HS_CONSTANT_DATA_OUTPUT input, 	float2 domain : SV_DomainLocation,	const OutputPatch<HS_CONTROL_POINT_OUTPUT, NUM_CONTROL_POINTS> patch)
{
    ...
    float2 vEdge = patch[3].uv - patch[0].uv;
    ...
    float2 yEdge = patch[3].position.xz - patch[0].position.xz;
    ...
    float yValue = g_Heightmap.SampleLevel(g_HeightmapSampler,OUT.uv,0.0f).r;
