...
// Sampler for the Density Data generated by noise.lib
SamplerState samplerPoint : register(s0);
//The texture containing the density values
Texture3D<float> densityTex : register(t1);
//Lookup Table of the marching cubes algorithm
Texture2D<int> SamplePointTex : register (t0);
...
cbuffer LookUp : register (b1)
{
	float4 samplePoint[8];
	float4 dataStep;
}

//Interpolate along a cubes edge to find the exact point for the vertex
...
int MarchingTableValue(int i, int j)
{
	if (i >= 256 || j >= 16)
	{
		return -1;
	}
    return SamplePointTex.Load(int3(j, i, 0));
}
...
void main(point GS_INPUT input[1], inout TriangleStream<GS_OUTPUT> triStream)
{
    ...
    //Sample the desntiy value using sampler and the sampling points
    for (int i = 0; i < 8; i++)
	{
        cubePoses[i] = position.xyz + samplePoint[i].xyz;
	...
	if (cubeindex != 0 && cubeindex != 255)
	{
		...
		float3 vector1;
		float3 vector2;
        for (int i = 0; MarchingTableValue(cubeindex, i) != -1; i += 3)
		{
			//Add vertices to the output stream
            output.Pos = float4(vertlist[MarchingTableValue(cubeindex, i + 0)], 1);
			worldPos1 = output.Pos;
			output.Pos = getProjectionPos(output.Pos);
			point1 = output;
			
            output.Pos = float4(vertlist[MarchingTableValue(cubeindex, i + 1)], 1);
			worldPos2 = output.Pos;
			output.Pos = getProjectionPos(output.Pos);
			point2 = output;
				
            output.Pos = float4(vertlist[MarchingTableValue(cubeindex, i + 2)], 1);
